...
class FraudDetector:
    ...
    async def check_listing(self, seller_id: int, price: Decimal, condition: str) -> tuple[bool, list[str]]:
        """
    ...
    async def check_purchase(
        self, buyer_id: int, seller_id: int, listing_id: str, price: Decimal
    ) -> tuple[bool, str | None]:
        """
    ...
    async def check_offer(
        self, listing_id: str, buyer_id: int, seller_id: int, offer_price: Decimal
    ) -> tuple[bool, str | None]:
        """
        ...
        # Get listing details
        listing = await db.fetch_one('SELECT price FROM marketplace_listings WHERE id = $1', listing_id)
    ...
    async def validate_transaction_completion(self, transaction_id: int) -> tuple[bool, str | None]:
        """
        ...
        listing = await db.fetch_one('SELECT status FROM marketplace_listings WHERE id = $1', transaction['listing_id'])
    ...
    async def _flag_fraud(self, listing_id: str | None, user_id: int, flag_type: str, severity: str, details: dict):
    ...
    async def get_user_risk_score(self, user_id: int) -> tuple[int, list[dict]]:
        ...
        severity_weights = {'low': 10, 'medium': 25, 'high': 50}

        for flag in user_flags:
            weight = severity_weights.get(flag['severity'], 10)
            risk_score += weight
            flags.append({'type': flag['flag_type'], 'severity': flag['severity'], 'date': flag['created_at']})
    ...
    async def is_user_blocked(self, user_id: int) -> tuple[bool, str | None]:
