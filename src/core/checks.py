import logging
import urllib.parse
from src.config.config import DATABASE_URL, DISCORD_TOKEN, ADMIN_ALERT_CHANNEL_ID
...
class StartupChecks:
    ...
    async def check_admin_channel() -> dict:
        if not ADMIN_ALERT_CHANNEL_ID:
            return {"name": "Admin Channel", "status": "WARN", "message": "ADMIN_ALERT_CHANNEL_ID is not set. Operational alerts will not be delivered."}
