...
class AdminNotifier:
    ...
    async def _get_channel(self):
        ...
        if self._channel is None:
            self._channel = self.bot.get_channel(ADMIN_ALERT_CHANNEL_ID)
            if self._channel is None:
                try:
                    self._channel = await self.bot.fetch_channel(ADMIN_ALERT_CHANNEL_ID)
                except Exception as e:
                    logger.warning("Failed to fetch admin alert channel: %s", e)
                    
        return self._channel

    def _should_alert(self, dedupe_key: str, cooldown_minutes: int) -> bool:
        if not dedupe_key:
            return True
    ...
    async def send_alert(self, title: str, description: str, severity: str = "INFO", dedupe_key: str = None, cooldown_minutes: int = 60):
