...
from src.utils.security import rate_limit, sanitize, InputValidator
...
class MarketplaceEnhanced(commands.Cog):
    ...
    async def search_items(self, ctx, *, query: str):
        ...
        try:
            ...
            if len(listings) > 10:
                embed.set_footer(text=f"Showing 10 of {len(listings)} results. Refine your search for more specific results.")
    ...
    async def add_to_watchlist(self, ctx, listing_id: str):
        ...
        try:
            ...
            listing = await db.fetch_one(
                "SELECT title, price FROM marketplace_listings WHERE id = $1",
                listing_id
            )
    ...
    async def view_watchlist(self, ctx):
        ...
        try:
            ...
            for item in items:
                ...
                embed.add_field(
                    name=f"{item['emoji']} {item['title'][:50]}",
                    value=value,
                    inline=False
                )
    ...
    async def make_offer(self, ctx, listing_id: str, price: str, *, message: str = ""):
        ...
        try:
            ...
            is_clean, fraud_reason = await fraud_detector.check_offer(
                listing_id, buyer['id'], listing['seller_id'], offered_price
            )
            ...
            await db.execute(
                """INSERT INTO marketplace_offers 
                   ...
                   VALUES ($1, $2, $3, $4, $5)""",
                listing_id, buyer['id'], offered_price, 
                sanitize(message, max_length=500), expires
            )
        ...
        try:
            ...
            if received_offers:
                ...
                for offer in received_offers[:5]:
                    ...
                    time_left = offer['expires_at'] - datetime.utcnow()
                    offers_text += f"• **{offer['title'][:30]}**: ${offer['offered_price']} (from {buyer_name})\n"
                
                embed.add_field(
                    name=f"📥 Received ({len(received_offers)})",
                    value=offers_text or "No pending offers",
                    inline=False
                )
    ...
    async def view_featured(self, ctx):
        ...
        try:
            ...
            for i, listing in enumerate(featured[:5], 1):
                ...
                embed.add_field(
                    name=f"#{i} {listing['emoji']} {listing['title'][:40]}",
                    value=value,
                    inline=False
                )
            ...
            for listing in expiring:
                ...
                if seller:
                    try:
                        ...
                        embed.add_field(
                            name="Action",
                            value=f"Use `!bump {listing['id']}` to refresh it!",
                            inline=False
                        )
