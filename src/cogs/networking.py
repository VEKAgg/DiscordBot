...
class Networking(commands.Cog):
    ...
    async def profile(self, ctx, member: nextcord.Member = None):
        """View your or another member's professional profile."""
        target = member or ctx.author
        profile = await self.svc.get_profile(str(target.id))

        if not profile:
            msg = (
                'You have not set up a profile yet. Use `!setupprofile` to create one.'
            ...
            for field, label in [
                ...
                embed.add_field(name=label, value=profile.get(field, 'Not set'),
                                inline=False)
    ...
    async def setupprofile(self, ctx):
        ...
        try:
            ...
            embed = nextcord.Embed(
                ...
                description=(
                    "Let's set up your professional profile. I will ask a few questions.\n"
                    "Type 'skip' to skip a question or 'cancel' to abort."
                ),
                color=nextcord.Color.blue()
            )
            await ctx.send(embed=embed)
            ...
            questions = [
                ('What is your professional title?', 'title'),
            ...
            await ctx.send('An error occurred while setting up your profile. Please try again.')
    ...
    async def connect(self, ctx, member: nextcord.Member, *, message: str = None):
        ...
        if member == ctx.author:
            await ctx.send('You cannot connect with yourself.')
            return
        try:
            if await self.svc.connection_exists(str(ctx.author.id), str(member.id)):
                await ctx.send('You are already connected with this member.')
                return

            await self.svc.create_request(
                str(ctx.author.id), str(member.id), message or ''
            )
            ...
            await ctx.send('An error occurred while sending the connection request.')
    ...
    async def accept(self, ctx, member: nextcord.Member):
        ...
        try:
            request = await self.svc.get_pending_request(
            ...
            try:
                notify = nextcord.Embed(
                    ...
                    description=f'{ctx.author.mention} accepted your connection request.',
                    color=nextcord.Color.green()
                )
    ...
    async def decline(self, ctx, member: nextcord.Member):
        ...
        try:
            ...
            embed = nextcord.Embed(
                ...
                description=f'You have declined the request from {member.mention}.',
            ...
            try:
                notify = nextcord.Embed(
                    title='Connection Request Declined',
                    description=f'{ctx.author.mention} declined your connection request.',
                    color=nextcord.Color.red()
                )
                await member.send(embed=notify)
            except nextcord.Forbidden:
    ...
    async def connections_cmd(self, ctx):
        """View all of your current connections."""
        try:
            user_connections = await self.svc.get_connections(str(ctx.author.id))
            if not user_connections:
                await ctx.send(
                    'You have no connections yet. Use `!connect @user` to connect with someone.'
                )
                return
            ...
            for conn in user_connections:
                other_id = (
                    conn['user2_id']
                    if conn['user1_id'] == str(ctx.author.id)
                    else conn['user1_id']
                )
