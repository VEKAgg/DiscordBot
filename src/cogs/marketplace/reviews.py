...
class MarketplaceReviews(commands.Cog):
        ...
        try:
            ...
            await db.execute(
                """INSERT INTO marketplace_reviews 
                   ...
                   VALUES ($1, $2, $3, $4, $5, $6)""",
                transaction_id, user['id'], reviewee_id, rating, safe_comment, is_buyer_review
            )
            ...
            # Notify reviewee
            reviewee_discord_id = transaction['seller_discord_id'] if is_buyer_review else transaction['buyer_discord_id']
    ...
    async def view_seller_profile(self, ctx, member: nextcord.Member = None):
        ...
        try:
            ...
            embed = nextcord.Embed(
                title=f"🏪 {target.display_name}'s Seller Profile",
            ...
            if reviews:
                ...
                for review in reviews:
                    ...
                    stars = "⭐" * review['rating']
                    comment = review['comment'][:50] + "..." if review['comment'] and len(review['comment']) > 50 else (review['comment'] or "No comment")
            ...
            if active:
                listings_text = "\n".join([f"{item['emoji']} {item['title'][:30]} - ${item['price']}" for item in active])
    ...
    async def view_my_reviews(self, ctx):
        ...
        try:
            ...
            for review in reviews[:5]:
                ...
                embed.add_field(
                    name=f"For: {review['title'][:30]} - {name}",
                    value=value,
                    inline=False
                )
    ...
    async def bump_listing(self, ctx, listing_id: str):
        ...
        try:
            ...
            await db.execute(
                "UPDATE marketplace_listings SET bumped_at = NOW() WHERE id = $1",
                listing_id
            )
