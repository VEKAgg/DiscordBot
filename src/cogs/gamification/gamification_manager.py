...
from src.database.sqlite_db import User, get_session
import math
...
class GamificationManager(commands.Cog):
    ...
    async def get_or_create_user(self, user_id: str):
        ...
        async for session in get_session():
            ...
            result = await session.execute(
                select(User).where(User.discord_id == str(user_id))
            )
    ...
    async def award_points(self, user_id: str, action: str, bonus: int = 0):
        ...
        async for session in get_session():
            ...
            new_level = math.floor(1 + math.log(user.experience / 100 + 1, 2))
            ...
            if new_level > old_level:
                ...
                if discord_user:
                    ...
                    embed.add_field(name="Total Points", value=str(user.points))
    ...
    async def profile(self, ctx, member: nextcord.Member = None):
        ...
        async for session in get_session():
            ...
            current_level_exp = 100 * (pow(2, user.level - 1) - 1)
            next_level_exp = 100 * (pow(2, user.level) - 1)
            exp_progress = user.experience - current_level_exp
            ...
            embed.add_field(name="Total Points", value=str(user.points), inline=True)
            embed.add_field(name="Experience", value=f"{user.experience:,}", inline=True)
    ...
    async def leaderboard(self, ctx):
        ...
        async for session in get_session():
            ...
            for i, user in enumerate(top_users, 1):
                ...
                embed.add_field(
                    name=f"{i}. {name}",
                    value=f"Level {user.level} | {user.points:,} points",
                    inline=False
                )
